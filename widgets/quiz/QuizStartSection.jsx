...
export default function QuizStartSection({
  ...
  const StartButton = () => {
    ...
    if (typeof onStart === 'function') {
      return (
        <button type="button" onClick={onStart} style={btnStyle}>
          시작하기
        </button>
      );
    }
    ...
    if (!slug) {
      return (
        <button type="button" disabled style={{ ...btnStyle, opacity: 0.6 }}>
          슬러그 없음
        </button>
      );
    }

    // slug가 주어졌을 때 기본 링크로 시작
    return (
      <Link href={`/fun/quiz/${slug}/q/1`} style={btnStyle}>
        시작하기
      </Link>
    );
  };
